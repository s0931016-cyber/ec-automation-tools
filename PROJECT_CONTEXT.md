...
実装では `createOrReplaceInventoryItem` と `createOffer` までを行い、`publishOffer` は明示的な確認画面と実行ボタンができるまで絶対に実行しない。
...
重要な仕様変更があったら、この `PROJECT_CONTEXT.md` または `TASKS.md` に追記する。

Codexには毎回、以下のように指示するとよい。

```text
PROJECT_CONTEXT.md と TASKS.md を読んでから作業してください。
まずは現在の優先タスクを実装してください。
```
